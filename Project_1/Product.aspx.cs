    ...
    public partial class Product : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (Session["pid"] != null)
                {
                    string sel = "select * from products where p_id='" + Session["pid"] + "'";
                    ...
                    while (dr.Read())
                    {
                        ...
                        Label4.Text = dr["p_stock"].ToString();
                    }
                }
            }
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            if (qty <= stock)
            {
                string cart_ins = "insert into cart_table values(" + n_cart + "," + Session["pid"] + "," + Session["user_id"] + "," + qty + "," + tot_price + ")";
        ...
        {
            string get_stock = "select p_stock from products where p_id='" + Session["pid"] + "'";
        ...
        {
            string get_price = "select p_price from products where p_id='" + Session["pid"] + "'";
