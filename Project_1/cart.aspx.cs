...
namespace Project_1
{
    public partial class cart : System.Web.UI.Page
    {
        ...
        {
            string sel = "select * from cart_table join products on cart_table.p_id=products.p_id";
        ...
        protected void GridView1_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            ...
            string strup = "update cart_table set p_qty=" + newqty + " where cart_id=" + getid + "";
            int n = cls.fun_exenonquery(strup);
            if (n == 1)
            {
                string nqnty = qtyupdate.Text;
                string op = "select p_id from cart_table where cart_id=" + getid + "";
                string proid = cls.fun_exescalar(op).ToString();

                string pri = "select p_price from products where p_id=" + proid + "";
                double npri = Convert.ToDouble(cls.fun_exescalar(pri));

                int nqnty1 = Convert.ToInt32(nqnty);
                string mp = "update cart_table set p_price=" + nqnty1 * npri + " where cart_id=" + getid + "";
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 1; i <= ns; i++)
            {
                string sel = "select * from cart_table join products on cart_table.p_id=products.p_id where cart_id=" + i + " ";
            ...
            {
                string ins = "insert into [order] values(" + Session["user_id"] + "," + item.pid2 + ",'" + item.pname + "'," + item.gp + "," + item.proqnty + ",'" + dt + "','available')";
                int a = cls.fun_exenonquery(ins);

                string del = "delete from cart_table where p_id = " + item.pid2;
